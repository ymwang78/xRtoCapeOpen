...
| `evaluateConstraints` | `GetMINLPNonlinearConstraintValues(cids)` | 长度 0 / 无约束 → 直接返回 0，不发远程调用 |
| `evaluateObjectiveGradient` | `GetMINLPObjectiveFunctionDerivativeValues("Nonlinear", v)` | 顺序同 `getObjectiveGradientStructure`；**结构为空（目标恒为常数的可行性问题）或长度 0 → 返回 0**。宿主按结构长度开空 vector 传 `data()`，那是 `nullptr`，不能当错误参数拒绝——否则 `xOptProblemComp` 断言失败、RSQP 报 "Fail to get linearized objective" |
| `evaluateConstraintsJacobianValues` | `GetMINLPConstraintDerivativeValues("Jacobian", cids, vals)` | 顺序同 `getConstraintJacobianStructure`；长度 0 / 结构为空 → 返回 0 |
...
### 5.2 真实 COM ABI（从官方 CAPE-OPEN 类型库提取，2026-06 核对）

来源：CO-LaN CAPE-OPEN Tester Suite 2.4.6 随附的 `CAPE-OPENv1-1-0.tlb`
（安装于 `C:\Program Files (x86)\COTester 2.4.6\`）。经 MSVC `#import` 生成 `.tlh` 核对：

**官方 IID（COM）**
| 接口 | IID |
|------|-----|
| `ICapeMINLP`（模型） | `{678C09CC-7D66-11D2-A67D-00105A42887F}` |
| `ICapeMINLPSystem`（求解器） | `{678C09CD-7D66-11D2-A67D-00105A42887F}` |
| `ICapeMINLPSolverManager`（工厂） | `{678C09CE-7D66-11D2-A67D-00105A42887F}` |

**关键 ABI 事实（修正前文若干假设）**
1. **数组一律是 `VARIANT`(内含 `SAFEARRAY`)**，不是裸 `SAFEARRAY`。例：
   `raw_GetMINLPVariableBounds([in] VARIANT vids, [in,out] VARIANT* LB, [in,out] VARIANT* UB)`。
   故 §6.4 的 marshaling 单元应为 `VARIANT(VT_ARRAY|VT_R8)`（double 数组）/`VT_I4`（long 索引）。
2. `raw_GetMINLPSize` = 10 个 `[in,out] long*`，与 `CapeMINLPSize` 字段一一对应。
3. `raw_GetMINLPStructure([in] BSTR structuretype, [in,out] VARIANT* rowindex, columnindex, objindex)`。
4. **`ICapeMINLPSystem` 极简**：只有 `Solve()` 和 `get_parameters()`（返回参数集合 `IDispatch`/`ICapeCollection`）。
   **没有 CORBA IDL 里的 `SetParameters`/`SetProblemID`**——参数经 CAPE-OPEN 参数集合模式配置。
5. 接口是 **dual（派生自 `IDispatch`）**；可经 vtable 直接调 `raw_*`（custom）方法。
6. 真实 `ICapeMINLP` 还有 Hessian / Lagrange / VariableTypes / ConstraintLinearity 等额外方法——
   我们的 `ICapeMINLPModel` 抽象只取子集，COM 后端不调用额外项即可。

> 实现策略：COM 后端用 `#import "CAPE-OPENv1-1-0.tlb"`（或据此手写一份精简接口头，仅含我们用到的方法）
> 取得精确 vtable，再在 `CapeMINLPModelCom` 内做 `VARIANT/SAFEARRAY/BSTR/HRESULT` ↔ `std::vector`/UTF-8 转换。
> 类型库属 CO-LaN 资产，**不入库**；仓库只引用其路径/IID。

### 5.3 测试对端现状（重要）
- **Multiflash `MF_CAPEOPEN64.dll` 不可用**：是 CAPE-OPEN 热力学组件（`ICapeThermo*`），不实现 MINLP。
- CO Tester 2.4.6 内含 MINLP 组件（`MINLPSolverCOTester.MINLPWrapper`、`RegisterMINLPSolver.RegisterMINLP`），
  但**全是 32 位（x86）**，且面向「测试求解器」（它当 PME 提供测试模型、驱动被测 `ICapeMINLPSystem`），
  不是我们 x64 后端可直接 in-proc 调用的「`ICapeMINLP` 模型组件」。
- 结论：**自建一个 x64 参考 COM 组件**（用上表官方 IID 实现 `ICapeMINLP`，包 Mock NLP）作为 COM 后端的
  测试对端；CO Tester（32 位）留作合规性旁路验证。
...
### 6.4 数据编解码（COM 后端）
按 §5.2 实测 ABI，**数组是 `VARIANT` 包 `SAFEARRAY`**：
`std::vector<double>`↔`VARIANT(VT_ARRAY|VT_R8)`；`std::vector<int>`↔`VARIANT(VT_ARRAY|VT_I4)`（索引/ids）；
名称数组↔`VARIANT(VT_ARRAY|VT_BSTR)`（UTF-16↔UTF-8，走 `zce::CharacterConvertor`）。
`[in,out] VARIANT*` 输出参数：被调方分配 SAFEARRAY，调用方 `VariantClear` 释放。
字符串生命周期：返回给 `xOptProblemT` 的 `const char*[]` 必须指向 DLL 内稳定缓存
（`std::vector<std::string>`），与 BlackBox 同。
...
- [x] 手写精简接口头 `CapeOpenComInterfaces.h`（官方 IID + `ICapeMINLP` 前 23 方法精确 vtable，§5.2）。
- [x] `CapeVariantMarshal`（`VARIANT(VT_ARRAY|VT_R8/I4/BSTR)` ↔ `std::vector`/UTF-8，含 `BstrGuard`/`VariantGuard`）。
- [x] `CapeMINLPModelCom`：`CoInitializeEx`/`CoCreateInstance`(ProgID 或 `{CLSID}`)/`QI ICapeMINLP`、
      `HRESULT`→负返回码；另有注入构造（传 `ICapeMINLP*`）供 in-proc 单测。工厂注册 `com:` scheme。
- [x] **测试对端**：自建 x64 参考组件 `RefCapeMINLP`（官方 IID 实现 `ICapeMINLP`，包 Mock NLP），
...
- [x] `CapeCorbaMarshal`（CORBA `CapeArray*` sequence ↔ `std::vector`/UTF-8）。
- [x] `CapeMINLPModelCorba`：`ORB_init`/`string_to_object`/`_narrow`（注入构造供 collocated 测试），
      `CORBA::Exception`→负返回码；工厂注册 `corba:` scheme（`CapeRegisterCorbaBackend`）。
- [x] IDL 由 CMake 用 vcpkg `tao_idl`（4.0.5）从 `SqpSolver.idl` **重新生成**到 build 目录
      （`-Yp,${CMAKE_CXX_COMPILER}` 指定预处理器），不入库。
- [x] **测试对端**：参考 POA 服务端 `RefCapeMINLPServant`（实现 `SqpSolver::ICapeMINLP`，包 Mock），
      经 collocated `ORB`/`POA` 全链路对拍 + 经 `CapeMINLPProblemCore` 驱动 `xOptProblemT`。
      `tests/test_capeopen_corba.cpp` **2/2 通过**（vcpkg `ace[tao]` x64-windows-static-md，静态 TAO）。
